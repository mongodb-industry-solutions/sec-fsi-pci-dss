...
const DEFAULT_SEEN_KEY = 'psp_debug_hint_seen';
 ...
 * REST_MS, and repeats until `dismissHint` retires it permanently. `seenKey` scopes the "already
 * seen" flag, so each screen carrying a debug toggle hints on its own first visit.
 */
export function useDebugHint(seenKey: string = DEFAULT_SEEN_KEY) {
