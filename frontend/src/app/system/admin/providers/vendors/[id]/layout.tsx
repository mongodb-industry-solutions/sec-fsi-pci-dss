...
function LayoutInner({ children }: { children: React.ReactNode }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      router.push(TYPE_CATEGORY_PATH[integration.externalProviderArrangementType] ?? '/system/admin/providers');
    } catch (err) {
  ...
  if (!integration) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-3">
        ...
        <Link href="/system/admin/providers" className="text-xs text-gray-400 hover:text-gray-700 underline flex items-center gap-1">
  ...
  const categoryPath = TYPE_CATEGORY_PATH[integration.externalProviderArrangementType] ?? '/system/admin/providers';
      ...
      <div className="flex">
        ...
        <nav className="w-44 shrink-0 border-r bg-white min-h-[calc(100vh-88px)] px-2 py-4 space-y-0.5 sticky top-[88px] self-start">
          {NAV.map(item => {
            ...
            const href = `/system/admin/providers/vendors/${id}/${item.path}`;
