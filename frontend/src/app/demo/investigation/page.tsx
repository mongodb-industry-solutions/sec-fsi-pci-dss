...
export default function InvestigationPage() {
  ...
  useEffect(() => { loadCases(); }, [filterStatus, filterSeverity]); // eslint-disable-line
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-5">
        ...
        <div className="bg-white rounded-xl border p-4">
          <div className="flex gap-3">
            <select value={searchField} onChange={(e) => setSearchField(e.target.value as SearchField)} className="border rounded-lg px-3 py-2 text-sm">
            ...
            <input type="text" value={searchValue} onChange={(e) => setSearchValue(e.target.value)}
              placeholder="Search value…" className="flex-1 border rounded-lg px-3 py-2 text-sm" />
            <button onClick={loadCases} className="bg-[#001E2B] text-[#00ED64] px-4 py-2 rounded-lg text-sm font-semibold">
              Search
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-3 items-center text-sm">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="border rounded-lg px-3 py-1.5">
            <option value="">All Status</option>
            {['open','under_review','escalated','resolved_cleared','resolved_fraud','closed'].map(s => (
              <option key={s} value={s}>{s.replace(/_/g,' ')}</option>
            ))}
          </select>
          <select value={filterSeverity} onChange={(e) => setFilterSeverity(e.target.value)} className="border rounded-lg px-3 py-1.5">
