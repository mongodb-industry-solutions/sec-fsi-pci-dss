...
export function EncryptedKycSearch({ token, role, escalationToken }: Props) {
  ...
  // Debounced auto-search. Client validation gates the request; the server re-validates.
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      ...
      setSearchError(null);
      try {
        const res = await api.customer.search(body, token, escalationToken);
  ...
  }, [bodyKey, token, escalationToken]); // eslint-disable-line react-hooks/exhaustive-deps

  if (registryError) {
    return <div className="text-sm text-red-600">Could not load searchable fields: {registryError}</div>;
  }
  if (!fields.length) {
    return <div className="text-sm text-gray-400">Loading searchable fields…</div>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Results */}
      {loading ? (
        <div className="text-center py-8 text-gray-400 text-sm">Searching over encrypted data…</div>
      ) : searchError ? (
            ...
            <tbody className="divide-y">
              {results.map((r) => (
                <tr key={r.customerAgreementInstanceReference} className="hover:bg-gray-50">
                  <td className="px-3 py-2 font-medium text-gray-900">{r.customerName || <Restricted />}</td>
