...
export function MonthlyBars({ title, data, color = 'bg-[#00ED64]', unit }: {
  title: string;
  data: Array<{ year: number; month: number; count: number }>;
  color?: string;
  /** Noun for the tooltip (e.g. "case", "payment"). Defaults to the title's subject. */
  unit?: string;
}) {
  const max = Math.max(1, ...data.map((d) => d.count));
  // Derive the noun from the title ("Cases by month" → "cases") when not given.
  const noun = (unit ?? title.replace(/\s*(by|per)\s+month\s*$/i, '').trim().toLowerCase()) || 'items';
  return (
        ...
        <div className="flex items-end gap-3 h-40">
          {data.slice(-12).map((d) => (
            <div key={`${d.year}-${d.month}`} className="flex-1 flex flex-col items-center gap-1 min-w-0">
              ...
              <div
                className={`w-full ${color} rounded-t ring-1 ring-inset ring-black/20`}
                style={{ height: `${Math.max(4, (d.count / max) * 120)}px` }}
                title={`${FULL_MONTHS[d.month - 1]} ${d.year}, ${countLabel(d.count, noun)}`}
              />
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((it) => {
            const pct = Math.round((it.value / denom) * 100);
            return (
              <div key={it.label} className="flex items-center gap-2" title={`${it.label}: ${it.value} of ${total} (${pct}%)`}>
