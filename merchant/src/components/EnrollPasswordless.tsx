...
export default function EnrollPasswordless({
  ...
  const remove = async () => {
    ...
    try {
      ...
      setEnrolled(false);
      setMsg({ tone: 'ok', text: 'Removed from this browser. Revoke it at Securit4 Pay to disable it everywhere.' });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section className="glass rounded-2xl p-5">
      ...
      <p className="mt-1 text-sm text-muted">
        ...
        sent to Securit4 Pay. The private key is non-extractable, stays in this browser, and is never exported.
      </p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <a href={credentialsUrl} target="_blank" rel="noreferrer" className="btn-ghost inline-flex items-center gap-1.5 text-sm">
          <ExternalLink className="h-4 w-4" aria-hidden /> Manage keys at Securit4 Pay
        </a>
      </div>
    </section>
  );
