...
export async function getHeldExecution(db: Db, executionRef: string): Promise<PaymentExecutionProcedure | null> {
  const exec = await db.collection<PaymentExecutionProcedure>(PAYMENT_EXECUTION_COLLECTION)
    .findOne({ paymentExecutionInstanceReference: executionRef, paymentExecutionStatus: 'pending' });
  if (!exec) return null;
...
export async function submitHeldTransfer(db: Db, executionRef: string): Promise<boolean> {
  const exec = await getHeldExecution(db, executionRef);
...
export async function reverseHeldTransfer(db: Db, executionRef: string): Promise<boolean> {
  const exec = await getHeldExecution(db, executionRef);
  if (!exec) return false;
