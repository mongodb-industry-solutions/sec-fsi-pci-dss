...
export const swaggerPlugin = fp(async function (fastify: FastifyInstance) {
  ...
  await fastify.register(fastifySwagger, {
    openapi: {
      openapi: '3.0.0',
      info: {
        title: 'Securit4 Pay (PSP Platform API)',
        version: '1.0.0',
        description: `
## Overview

REST API for the **Securit4 Pay** (PSP Platform Demo). It demonstrates how
...
- **QE:none**: encrypted client-side, not searchable; returned only with the correct DEK
  (Data Encryption Key). Requires \`level2_investigator\` role.
      ...
      tags: [
        ...
        { name: 'auth:enrollment', description: 'module:identity · SD-91/SD-16 passwordless credential enrollment (WebAuthn-style). /api/v1/auth/enroll. Register/list/rotate/revoke public-key authenticators. Session-gated, owner-scoped.' },
        { name: 'auth:ciba',    description: 'module:identity · OIDC Client-Initiated Backchannel Authentication. /api/v1/auth/bc-authorize + ciba grant on /token. Passwordless, no browser redirect; assertion-authenticated approval.' },
    ...
    theme: {
      ...
      css: [{
        ...
        content: [
          ...
          /* Append "Securit4 Pay" label after the icon */
          '.topbar-wrapper .link::after { content: "Securit4 Pay"; font-size: 1.15rem; font-weight: 700; color: #00ED64; margin-left: 8px; letter-spacing: 0.01em; }',
        ].join('\n'),
      }],
    },
    uiHooks: {
