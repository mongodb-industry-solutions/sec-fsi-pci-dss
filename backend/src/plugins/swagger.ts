  ...
  (Data Encryption Key). Requires \`level2_investigator\` role.
      ...
      tags: [
        ...
        { name: 'system',       description: 'module:system — /api/v1/system. Health check (public) + raw document viewer (non-production, JWT required).' },
        {
          name: 'admin',
          description: [
            'module:admin — /api/v1/admin. Administration panel: setup commands, log streaming, system info.',
            '',
            '## Authentication flow',
            '',
            '1. `POST /api/v1/admin/login` with `{ username, password }` (no Bearer token required).',
            '2. The server computes `sha256(password)` and compares it against the `ADM_PASS` environment variable (which stores the SHA-256 hash of the admin password). The username is compared against `ADM_USER`.',
            '3. On success, a short-lived JWT (`role: "admin"`, 4 h TTL) is returned.',
            '4. Pass this JWT as `Authorization: Bearer <token>` to all other `/admin/*` endpoints.',
            '',
            '## Default credentials (dev only)',
            '',
            '| Variable | Default | Meaning |',
            '|---|---|---|',
            '| `ADM_USER` | `admin` | Admin username |',
            '| `ADM_PASS` | SHA-256(`admin`) | SHA-256 hash of the admin password |',
            '',
            'To change the password, set `ADM_PASS` to `sha256(<your-password>)` in `.env`.',
            '',
            '## Cross-platform compatibility',
            '',
            'Command execution (`/run`, `/exec`) uses `child_process.spawn` with `shell: true`,',
            'delegating to `cmd.exe` on Windows and `/bin/sh` on Linux/macOS. Both endpoints',
            'stream output line-by-line via Server-Sent Events (SSE).',
          ].join('\n'),
        },
      ],
    },
