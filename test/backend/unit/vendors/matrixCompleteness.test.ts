...
describe('v37 P10.2: the ownership matrix describes the whole system', () => {
  ...
  it('claims every collection the PSP setup creates', () => {
    const declared = declaredIn('backend/src/vendors/setup/createCollections.ts', ['backend/src', 'packages/eventbus/src']);
  ...
  it('claims every collection the bank setup creates', () => {
    const declared = declaredIn('bankcore/src/vendors/setup/createCollections.ts', ['bankcore/src', 'packages/eventbus/src']);
