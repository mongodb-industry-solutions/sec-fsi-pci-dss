...
describe('v37 P4.6b: no bank is named in the PSP code', () => {
  const files = sourceFiles(resolve(ROOT, 'backend/src'))
    .concat(sourceFiles(resolve(ROOT, 'frontend/src')));
...
describe('v37 P4.6f: nothing branches on WHICH bank, only on whether one is linked', () => {
  ...
  const KNOWN_OFFENDERS: Array<{ file: string; phase: string; why: string }> = [
    {
      file: '/backend/src/modules/gateway/services/cardAuthorization.service.ts',
      phase: 'P6.1/P7.2',
  ...
  it('the funds gate branches on the account being LINKED, not on the bank', () => {
    const gate = readFileSync(resolve(ROOT, 'backend/src/providers/groups/providerGroups.ts'), 'utf8');
  ...
  it('the balance projection branches the same way', () => {
    const projection = readFileSync(
      resolve(ROOT, 'backend/src/modules/gateway/services/payoutAccountBalanceProjection.ts'), 'utf8',
    );
  ...
  it('the consent status decision switches on the standard enumeration, not on a bank', () => {
    const receiver = readFileSync(
      resolve(ROOT, 'backend/src/modules/provider/services/bankcoreNotification.service.ts'), 'utf8',
    );
