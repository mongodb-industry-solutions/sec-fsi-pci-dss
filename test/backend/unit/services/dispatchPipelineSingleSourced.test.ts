...
const DISPATCH = 'backend/src/modules/provider/services/integrationDispatch.service.ts';
...
describe('v37 P6.1: one dispatch path, with the resolver injected into it', () => {
  ...
  it('no other module resolves a provider for itself', () => {
    ...
    const callers = [
      'backend/src/modules/gateway/services/p2pTransfer.service.ts',
      'backend/src/modules/gateway/services/bankTransfer.service.ts',
      'backend/src/providers/groups/providerGroups.ts',
    ];
...
describe('v37 P6.3: the routing key per capability is declared, not inferred at each call site', () => {
  it('the entity-bound set is one table, in one place', () => {
    const resolver = source('backend/src/modules/provider/services/resolverStrategy.ts');
