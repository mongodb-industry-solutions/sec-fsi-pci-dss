...
import { selectPaymentProduct } from '../../../../backend/src/providers/payment-initiation/services/bankcorePis.client';
import { movementMethod } from '../../../../backend/src/modules/gateway/services/paymentMovement.service';
import { convert, DEFAULT_CURRENCY_EXCHANGE_CONFIG } from '../../../../backend/src/providers/currency-exchange/services/currencyExchange.service';
...
describe('v37 P5.7: a cross-currency transfer converts through the exchange capability', () => {
  ...
  it('the defaults are now SEEDED, so the margin is editable rather than compiled in', async () => {
    ...
    const seeds = JSON.parse(readFileSync(
      resolve(__dirname, '../../../../backend/data/capabilityModuleConfiguration.json'), 'utf8',
