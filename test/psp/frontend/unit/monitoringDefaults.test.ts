...
import { mergeMonitoringDefaults, type MonitoringServiceShape } from '../../../../psp/frontend/src/lib/monitoringDefaults';
...
describe('v37: monitoring defaults reconciliation', () => {
  ...
  it('the shipped file and the proxy agree on every default path', async () => {
    ...
    const shipped = JSON.parse(readFileSync(resolve(root, 'psp/frontend/public/monitoring-defaults.json'), 'utf8')) as {
      services: MonitoringServiceShape[];
    };
    const nextConfig = readFileSync(resolve(root, 'psp/frontend/next.config.js'), 'utf8');
