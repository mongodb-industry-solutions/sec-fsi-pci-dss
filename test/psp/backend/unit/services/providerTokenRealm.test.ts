...
describe('seeded provider token endpoints', () => {
  ...
  it('never resolves to the bankcore realm, which the authority does not have', () => {
    for (const record of credentialed) {
      expect(resolvedUnder('http://127.0.0.1:8085/realms/leafypay', record)).not.toContain('/realms/bankcore/');
  ...
  it('takes the realm from an issuer that already carries one', () => {
    for (const record of credentialed) {
      expect(resolvedUnder('http://authority.example/realms/leafypay', record))
        .toBe('http://authority.example/realms/leafypay/protocol/openid-connect/token');
    }
  });
  ...
  it('supplies the default realm when the issuer names none', () => {
    for (const record of credentialed) {
      expect(resolvedUnder('http://127.0.0.1:8085', record))
        .toBe('http://127.0.0.1:8085/realms/leafypay/protocol/openid-connect/token');
    }
  });

  it('does not double the realm segment under either issuer shape', () => {
    for (const record of credentialed) {
      for (const issuer of ['http://127.0.0.1:8085/realms/leafypay', 'http://127.0.0.1:8085']) {
  ...
  it('resolves to the standard OIDC token path', () => {
    for (const record of credentialed) {
      expect(resolvedUnder('http://127.0.0.1:8085/realms/leafypay', record))
