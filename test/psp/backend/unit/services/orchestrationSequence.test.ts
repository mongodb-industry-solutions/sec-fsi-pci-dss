...
describe('v37 N5: orchestration sequence baseline', () => {
  it('the settlement notification keeps every subscriber it has today', async () => {
    const { PayoutOrchestrationProcess } = await import('../../../../../psp/backend/src/modules/gateway/services/payoutOrchestration.process');
    const { RtpLifecycleProcess } = await import('../../../../../psp/backend/src/modules/gateway/services/rtpLifecycle.process');
  ...
  it('the post-transfer compliance process still hangs off the completion event', async () => {
    const { P2PComplianceProcess } = await import('../../../../../psp/backend/src/modules/gateway/services/p2pCompliance.process');
  ...
  it('the payment authorization saga keeps its subscription set', async () => {
    const { PaymentAuthorizationSaga } = await import('../../../../../psp/backend/src/modules/transaction/services/paymentAuthorization.saga');
  ...
  it('the post-authorization process keeps its subscription set', async () => {
    const { PostAuthorizationProcess } = await import('../../../../../psp/backend/src/modules/transaction/services/postAuthorization.process');
...
describe('v37 N5: the concurrent risk gate is not fused', () => {
  ...
  beforeEach(() => {
    dispatched.length = 0;
    vi.resetModules();
    vi.doMock('../../../../../psp/backend/src/modules/provider/services/integrationDispatch.service', () => ({
  ...
  it('dispatches fraud detection, sanctions and AML as three separate provider calls', async () => {
    const { screenTransfer } = await import('../../../../../psp/backend/src/modules/gateway/services/transferRiskGate');
