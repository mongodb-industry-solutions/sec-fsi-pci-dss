...
describe('v37 P5.3: the PSP does not credit a beneficiary', () => {
  it('the settlement handler returns before any balance movement when the bank executed', () => {
    const process = code('psp/backend/src/modules/gateway/services/payoutOrchestration.process.ts');
  ...
  it('the transfer path records the delegation BEFORE dispatching', () => {
    const transfer = code('psp/backend/src/modules/gateway/services/p2pTransfer.service.ts');
  ...
  it('the PSP asks the DEBTOR bank, never the creditor institution', () => {
    const client = source('psp/backend/src/providers/payment-initiation/services/bankcorePis.client.ts');
...
describe('v37 N1: the terminal state is the PSP process\'s decision, not a handler\'s', () => {
  it('the notification receiver assigns no terminal payment state', () => {
    const receiver = code('psp/backend/src/modules/provider/controllers/bankcoreNotification.controller.ts');
    const service = code('psp/backend/src/modules/provider/services/bankcoreNotification.service.ts');
