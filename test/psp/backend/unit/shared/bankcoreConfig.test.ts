...
const ENV_EXAMPLE = resolve(__dirname, '../../../../../psp/backend/src/vendors/setup/env.example');
...
async function loadConfig(overrides: Record<string, string | undefined>) {
  ...
  vi.resetModules();
  const mod = await import('../../../../../psp/backend/src/config');
...
describe('v37 P0.4/P0.6: bankcore configuration', () => {
  ...
  it('the bank database is always a separate database from the PSP one', async () => {
    ...
    vi.resetModules();
    const { config } = await import('../../../../../psp/backend/src/config');
  ...
  it('the keyvault namespace is the PSP KMS namespace, never the application database', async () => {
    ...
    vi.resetModules();
    const { config } = await import('../../../../../psp/backend/src/config');
    const { getKmsConfig } = await import('../../../../../psp/backend/src/vendors/encryption/kms');
