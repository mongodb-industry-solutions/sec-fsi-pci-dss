...
// has a flat field. Nothing in `psp/backend/src` knows it exists. If the same dispatch call reaches it in its own
...
describe('v37 P6.2a–c: a proprietary bank absorbed by configuration', () => {
  ...
  it('warns rather than silently doing nothing when a transform is not recognised', async () => {
    ...
    try {
      const { applyMappings } = await import('../../../../psp/backend/src/modules/provider/services/fieldMapping.service');
