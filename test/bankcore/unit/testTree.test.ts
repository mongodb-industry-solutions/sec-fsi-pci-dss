...
describe('v37 P0.8: bankcore test tree', () => {
  ...
  it('vitest includes both bankcore directories', () => {
    ...
    expect(cfg).toContain('test/bankcore/unit/**/*.test.ts');
    expect(cfg).toContain('test/bankcore/integration/**/*.test.ts');
  });
  ...
  it('the npm scripts name both directories', () => {
    ...
    expect(pkg.scripts['test:unit']).toContain('test/bankcore/unit');
    expect(pkg.scripts['test:integration']).toContain('test/bankcore/integration');
  });
});
