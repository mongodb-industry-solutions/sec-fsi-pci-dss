...
// `npm run setup:db --prefix psp/backend --reset`, where npm consumes `--reset` as one of ITS OWN options
...
describe('v37: reset wiring', () => {
  ...
  it('bankcore mirrors the backend reset entry', () => {
    expect(scripts('bank/backend')['setup:db:reset']).toContain('--reset');
    expect(scripts('psp/backend')['setup:db']).toContain('bin/setup.ts');
  });

  it('the root setup installs the shared packages and bankcore, or a fresh clone cannot reset', () => {
    const setup = scripts()['setup'];
    for (const workspace of ['packages/eventbus', 'packages/platform-links', 'psp/backend', 'bank/backend', 'psp/frontend', 'merchant']) {
  ...
  it('the PSP entry points orchestrate every bank, and the drop runs before the key vault goes', () => {
    const setupBin = readFileSync(resolve(ROOT, 'psp/backend/bin/setup.ts'), 'utf8');
    const seedBin = readFileSync(resolve(ROOT, 'psp/backend/bin/seed.ts'), 'utf8');
    const dropBin = readFileSync(resolve(ROOT, 'psp/backend/bin/setup-drop.ts'), 'utf8');
  ...
  it('no orchestrated script is gated on the kill switch', () => {
    ...
    const orchestrator = readFileSync(resolve(ROOT, 'psp/backend/src/vendors/setup/bankInstances.ts'), 'utf8');
    expect(orchestrator).not.toContain('config.bankcore.enabled');
  });

  it('the bank drop never touches the shared key vault', () => {
    const drop = readFileSync(resolve(ROOT, 'bank/backend/src/vendors/setup/dropAll.ts'), 'utf8');
