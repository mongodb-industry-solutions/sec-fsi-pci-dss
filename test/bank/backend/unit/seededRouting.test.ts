...
} from '../../../../bank/backend/src/modules/aspsp/services/bankIdentifier.service';
import type { BankProfileControlRecord } from '../../../../bank/backend/src/modules/aspsp/models/bankProfile.model';
import type { AccountArrangementControlRecord } from '../../../../bank/backend/src/modules/aspsp/models/accountArrangement.model';

const ROOT = resolve(__dirname, '../../../..');
const read = <T>(path: string): T => JSON.parse(readFileSync(resolve(ROOT, path), 'utf8')) as T;

const BANK = read<BankProfileControlRecord[]>('bank/backend/data/bankProfile.json')[0];
const BANK_ACCOUNTS = read<AccountArrangementControlRecord[]>('bank/backend/data/accountArrangements.json');
const HOLDERS = read<Array<{ accountHolderInstanceReference: string }>>('bank/backend/data/accountHolders.json');
...
const PSP_ACCOUNTS = read<PspAccount[]>('psp/backend/data/payoutAccounts.json')
...
describe('v37: every seeded card routes to the issuer', () => {
  it('the card seeder takes its BIN from the bank profile, not a bare network prefix', () => {
    ...
    const seeder = readFileSync(resolve(ROOT, 'bank/backend/src/vendors/seed/seedCardIssuer.ts'), 'utf8');
  ...
  it('the PSP no longer derives a card number at all', () => {
    const descoping = readFileSync(resolve(ROOT, 'psp/backend/src/vendors/seed/seedCardDescoping.ts'), 'utf8');
