...
function ensureDataFiles() {
  ...
  try {
    ...
    throw new Error(
      ...
      `  Run manually: npm run setup:data --prefix psp/backend`
    );
  }
}
...
export async function runSeed() {
  ...
  try {
    ...
    console.log('Seeding capabilityModuleConfiguration (internal Modules, ADR-029)...');
    await seedCapabilityModules(db);
