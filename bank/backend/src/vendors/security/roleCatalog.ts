 ...
 *
 * A near copy of the payment service's module of the same name, and the duplication is deliberate
 * for now: these are two independently deployed services with no shared runtime dependency today.
 * `packages/giam-client` already carries `effectivePermissions` and `createCatalogCache` for
 * exactly this, and consolidating both onto it is the follow-up. Recorded so the copy reads as a
