...
export async function runSetup(reset = false): Promise<void> {
  ...
  try {
    ...
    if (!reset) {
      ...
      if (orphans.length > 0) {
        throw new Error(
          ...
          + '  Rebuild it with:  npm run setup:db:reset --prefix bank/backend   (or npm run setup:reset from the repo root)',
        );
      }
    }
    ...
    console.log('  Seed it with: npm run setup:seed --prefix bank/backend');
  } finally {
    await closeQEClient();
  }
}
